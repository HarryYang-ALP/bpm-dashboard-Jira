...
知識庫：BPM網址 https://bpm.alp.global / 登入用 Azure AD Login+M365 Email / 代理人設定：Personal>Account>Leaving設Out of Office，Task Rules加Delegation規則 / Claim Task取得Share Task處理權 / 採購單SAP-MM判定：主營業務、資產、預付訂閱 / 一般採購LOA：3萬以下主管,3萬-30萬採購成控+主管,30萬-500萬+營運長,500萬-3000萬+執行長,3000萬以上+董事長 / 出差3工作天前申請，含住宿才用BPM`;
...
function adRenderBubbles(hint){{
  let set = AD_BUBBLES.default;
  for(const k of Object.keys(AD_BUBBLES)){{ if(k!=='default'&&hint.includes(k)){{set=AD_BUBBLES[k];break;}} }}
  document.getElementById('ad-bubble-wrap').innerHTML = set.map(q=>
    `<button class="ad-bubble-btn" onclick="adSendQ('${{q}}')">${{q}}</button>`).join('');
}}

function adSendQ(q){{ document.getElementById('ad-chat-input').value=q; adSendMsg(); }}

function adAddMsg(text, isUser){{
  const msgs = document.getElementById('ad-chat-messages');
...
async function adSendMsg(){{
  const inp = document.getElementById('ad-chat-input');
  const text = inp.value.trim();
  if(!text) return;
  inp.value = '';
  document.getElementById('ad-bubble-area').style.display='none';
  adAddMsg(text, true);
  adHistory.push({{role:'user',parts:[{{text}}]}});
  const tr = document.createElement('div');
  tr.className='ad-msg-row';
  const td=document.createElement('div');
  td.className='ad-typing';td.textContent='思考中...';
  tr.appendChild(td);
  document.getElementById('ad-chat-messages').appendChild(tr);
  document.getElementById('ad-chat-messages').scrollTop=99999;
  try{{
    const res = await fetch(AD_API_URL,{{method:'POST',headers:{{'Content-Type':'application/json'}},
      body:JSON.stringify({{system_instruction:{{parts:[{{text:AD_SYSTEM}}]}},contents:adHistory}})}});
    const data = await res.json();
    const reply = data.candidates?.[0]?.content?.parts?.[0]?.text||'抱歉，無法取得回應。';
    tr.remove(); adAddMsg(reply,false);
    adHistory.push({{role:'model',parts:[{{text:reply}}]}});
    document.getElementById('ad-bubble-area').style.display='block';
    adRenderBubbles(text);
  }}catch(e){{tr.remove();adAddMsg('發生錯誤，請稍後再試。',false);}}
}}
</script>
""", unsafe_allow_html=True)
